...
dict_loan_user = {'当日活跃用户': len(set(list(data_user[readDataDate]))),
                  '当月累计活跃用户': len(set(list(data_user[afterDate: orgDate]))),
                  '累计用户数': len(set(list(data_user[afterDate:]))),
                  '新增用户数': (len(set(list(data_user[afterDate:]))) - len(set(list(data_user[readDataDate:]))))}
...
dict_loan_amt = {'生意金-其他放款金额': (pos_amt + ck_amt +tx_amt +ft_amt + tl_amt)/10000,
                 '生意金-网商贷放款金额': syj_amt/10000,
                 '到手-放款金额': (ds_amt + jk_amt)/10000}
