...
# 将日报表各产品明细表按需汇总统计数据
# 源数据：通联钱包、到手下载统计对应日期，其他助贷产品放款及助贷用户下载当天报表，短信引流数据单独查询
...
# 用于生意金已累计放款数据查询
lastMonthDt = (datetime.datetime.strptime(readDataDate[:6] + '01', '%Y%m%d')
               + datetime.timedelta(days=-1)).strftime('%Y%m%d')  # 用于统计助贷用户月累计
lastYearDt = (datetime.datetime.strptime((readDataDate[:4] + '0101'), '%Y%m%d')
              + datetime.timedelta(days=-1)).strftime('%Y%m%d')  # 用于统计助贷用户年累计
...
if readDataDate[-4:] == '0101':
    ...
    df_wallet.loc['新增用户', '月累计'] = int(wallet_user.loc['合计：', '新增会员数'].replace(',', ''))
    df_wallet.loc['新增用户', '年累计'] = int(lastData.iloc[4, 5]) + int(wallet_user.loc['合计：', '新增会员数'].replace(',', ''))
else:
...
if readDataDate[-4:] == '0101':
    df_wallet.loc['短信引流', '月累计'] = int(msgUser)
    df_wallet.loc['短信引流', '年累计'] = int(msgUser)
elif readDataDate[-2:] == '01':
    df_wallet.loc['短信引流', '月累计'] = int(msgUser)
...
dict_loan_user = {'新增用户': len(list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(
    list(data_user.loc[beforeDate:, '客户手机号'].unique())),
                  '活跃用户': len(list(data_user.loc[readDataDate: beforeDate, '客户手机号'].unique()))}
df_loan_user = pd.DataFrame.from_dict(dict_loan_user, orient='index', columns=['数值'])
df_loan_user.index.name = '指标'
df_loan_user.loc['活跃用户', '月累计'] = len(list(data_user.loc[readDataDate: lastMonthDt, '客户手机号'].unique()))
df_loan_user.loc['活跃用户', '年累计'] = len(list(data_user.loc[readDataDate: lastYearDt, '客户手机号'].unique()))
if readDataDate[-4:] == '0101':
    df_loan_user.loc['新增用户', '月累计'] = len(list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(
        list(data_user.loc[beforeDate:, '客户手机号'].unique()))
    df_loan_user.loc['新增用户', '年累计'] = len(list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(
        list(data_user.loc[beforeDate:, '客户手机号'].unique()))
elif readDataDate[-2:] == '01':
    df_loan_user.loc['新增用户', '月累计'] = len(list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(
        list(data_user.loc[beforeDate:, '客户手机号'].unique()))
    df_loan_user.loc['新增用户', '年累计'] = int(lastData.iloc[7, 5]) + len(
        list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(list(data_user.loc[beforeDate:, '客户手机号'].unique()))
else:
    df_loan_user.loc['新增用户', '月累计'] = int(lastData.iloc[7, 4]) + len(
        list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(list(data_user.loc[beforeDate:, '客户手机号'].unique()))
    df_loan_user.loc['新增用户', '年累计'] = int(lastData.iloc[7, 5]) + len(
        list(data_user.loc[readDataDate:, '客户手机号'].unique())) - len(list(data_user.loc[beforeDate:, '客户手机号'].unique()))
