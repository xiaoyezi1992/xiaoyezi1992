...
# 三行业并筛选交易类型,增加笔数、金额、收益、产品，并修改联营商户收入所属方（行业分类汇总后手工匹配）
# 二级行业为信用卡中心、交易类型为转账扣款的交易金额、笔数归零
# 验证剔除快捷协议签约申请交易类型的交易明细
# 业务数据汇总简表，取字段部分明细
...
def time():
    daily_path = 'E:/数据/1-综合数据/TLT基础数据/每日明细/商户维度/'
    week_path = 'E:/数据/1-综合数据/TLT基础数据/每周明细/商户维度/'  # 与日报表差异处
    month_path = 'E:/数据/1-综合数据/TLT基础数据/月度明细/商户维度/'  # 与日报表差异处
    time_choose = input('请输入需汇总数据期间维度(日/周/月):')
    if time_choose == '日':
        return daily_path
    elif time_choose == '周':
        return week_path
...
# 交易类型判断函数
def tp_judge(val):
    tp_path = 'E:/数据/1-综合数据/TLT基础数据/判断条件.xlsx'
...
# 产品匹配函数
def pr_match(type):
    pr_path = 'E:/数据/1-综合数据/TLT基础数据/判断条件.xlsx'
...
# 分润判断、匹配函数
def prf_match(No):
    prf_path = 'E:/数据/1-综合数据/TLT基础数据/判断条件.xlsx'
...
# 开始写入新工作表
totalExcel = xlsxwriter.Workbook(totalPath + ('汇总(含个人){}.xlsx'.format(dateGet)))
...
# 所需部分字段明细
field_path = 'E:/数据/1-综合数据/TLT基础数据/判断条件.xlsx'
